...
- Clean, airy, precise, high-finish, distinctive, subject-grounded, spatially excellent, intensity-matched, readable, and not repetitive by default.
...
- Editorially composed where appropriate: deliberate rhythm, scale, focal hierarchy, negative space, and section sequencing.
- Premium but restrained: one standout move should carry the moment while surrounding elements stay disciplined.
- Materially coherent: backgrounds, surfaces, shadows, borders, media, icons, generated assets, and UI states should belong to one visual world.
...
- Hero or first viewport uses the subject's own materials, workflow, data shape, environment, live interaction, image, or claim as its focal point rather than a generic metric block or decorative visual.
...
- Run a two-pass critique before accepting the direction: first draft palette, type roles, layout concept, signature element, aesthetic risk, and AI-defaults to avoid; then name why rejected directions lost and revise anything that reads like the same design you would produce for an unrelated prompt.
...
Include these details in the concept brief whenever the surface is more than a small static section:

- State matrix: default, selected, hover/focus, empty, loading, error, success, disabled, modal/drawer, and mobile-collapsed states where relevant.
...
Before accepting a generated concept, reject or regenerate it if:

- It reads like a generic AI SaaS, portfolio, agency, fintech, productivity, or dashboard template.
...
- Sample content/data is fake-looking, shallow, or not credible for the product domain.
...
- For generated-concept work, before final handoff, use `view_image` on the accepted concept and latest browser screenshot in the same QA pass. This cannot be skipped or replaced with browser inspection or functional testing alone.
...
- Run a final polish pass after fidelity: judge the browser result as a standalone product design, remove weak filler, soften over-decoration, tighten spacing, fix awkward wraps, and repair details a senior designer would flag.
- Run the taste review gate: strongest visual moment, weakest visual moment, AI-slop / AI-default residue, typography issue, spatial issue, screenshot-worthy reason, and next three fixes.
