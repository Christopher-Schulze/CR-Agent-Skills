...
Default to intensity-matched, tasteful 8.5/10 finish and distinctiveness: exceptional enough to feel designed, restrained enough to build, and appropriate to the product type. Default visual posture is very clean, premium, calm, precise, high-end, and low-noise unless the user, brand, repo, or product category clearly asks for another vibe. Interpret clean polish as precise spacing logic, calm hierarchy, crisp typography, coherent density, disciplined components, and disciplined detail, not empty, sterile, generic, or under-designed.
...
- Motion storyboard: one entrance/reveal pattern, one interaction/state transition pattern, one optional product-tactility moment, timing/easing direction, and `prefers-reduced-motion` behavior. Motion must feel choreographed, not attached; every cue must map to hierarchy, state, product tactility, navigation, or comprehension.
...
19. Run the `references/quality-gates.md` premium finish, micro-detail polish, and taste review gates. Final response is blocked if the first impression does not feel premium for the intended surface, the strongest visual moment is weak, the identity test fails, first-use comprehension fails, or any raw/default component-library primitive still looks dropped in instead of designed.
